...
export function App(): React.JSX.Element {
  ...
  const onNodeClick = useCallback(
    (_event: React.MouseEvent, node: FlowNode) => setSelectedNodeId(node.id),
    [],
  );
    ...
    <main className="shell">
      ...
      <section className="metric-strip" aria-label="Runtime summary">
        ...
        <Metric
          icon={<Clock3 />}
          label="Avg latency"
          value={`${formatNumber(totals.avgLatency)} ms`}
        />
        <Metric
          icon={<Activity />}
          label="P95 latency"
          value={`${formatNumber(totals.p95Latency)} ms`}
        />
        <Metric
          icon={<CircleAlert />}
          label="Errors"
          value={formatInteger(totals.errors)}
          tone={totals.errors ? 'error' : undefined}
        />
        <Metric
          icon={<MemoryStick />}
          label="Heap used"
          value={runtime ? formatBytes(runtime.heapUsedBytes) : '—'}
        />
        <Metric
          icon={<Cpu />}
          label="CPU"
          value={runtime ? `${runtime.cpuPercent.toFixed(1)}%` : '—'}
        />
        <Metric
          icon={<Zap />}
          label="Event loop"
          value={runtime ? `${runtime.eventLoopUtilization.toFixed(1)}%` : '—'}
        />
        <Metric
          icon={<TimerReset />}
          label="Uptime"
      ...
      <section className={`workspace ${tracePanelOpen ? '' : 'traces-collapsed'}`}>
        ...
        <aside className="trace-panel">
          ...
          >
            <span>
              <GitBranch size={15} /> Recent traces
            </span>
          ...
          {tracePanelOpen && (
            <div className="trace-list">
              {snapshot.traces.length === 0 ? (
                <div className="empty-list">No requests captured yet.</div>
              ) : (
...
function ScopeNode({ data }: NodeProps<FlowNode>): React.JSX.Element {
  ...
  return (
    <div
      className={`scope-node type-${data.type} ${data.active ? 'active' : ''} ${data.errorCount ? 'has-error' : ''}`}
    >
      ...
      <div className="node-copy">
        ...
        <span className="node-stats">
          {data.requestCount} calls · {formatNumber(data.avgLatencyMs)} ms avg
        </span>
      </div>
  ...
  return (
    <aside className="drawer">
      ...
      <div className="detail-grid">
        ...
        <Detail
          label="Error rate"
          value={`${node.errorRate.toFixed(1)}%`}
          error={node.errorRate > 0}
        />
      ...
      <div className="dependency-list">
        {connected.length ? (
          connected.map((dependency) => (
            <div key={dependency.id}>
              <span>{dependency.name}</span>
              <small>{labelForType(dependency.type)}</small>
            </div>
          ))
        ) : (
...
function buildFlow(snapshot: TopologySnapshot): { nodes: FlowNode[]; edges: Edge[] } {
  ...
  const levelOf = (id: string, seen = new Set<string>()): number => {
    ...
    const level = parents.length
      ? 1 + Math.max(...parents.map((parent) => levelOf(parent, new Set(seen))))
      : 0;
    levels.set(id, level);
    return level;
  };
...
function calculateTotals(snapshot: TopologySnapshot): {
  requests: number;
  errors: number;
  avgLatency: number;
  p95Latency: number;
  ...
  const weightedLatency = routes.reduce(
    (sum, node) => sum + node.avgLatencyMs * node.requestCount,
    0,
  );
  return {
    requests,
