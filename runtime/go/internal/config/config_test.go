...
func TestLoadRejectsHTTPWithoutExplicitTypeScriptAuthority(t *testing.T) {
	t.Setenv("NODEFLOW_SINK", "http")
	if _, err := Load(); err == nil {
		t.Fatal("expected deprecated HTTP sink to require explicit TypeScript authority")
	}
}

func TestLoadAcceptsHTTPWithExplicitTypeScriptAuthority(t *testing.T) {
	t.Setenv("NODEFLOW_TOPOLOGY_ENGINE", "typescript")
	t.Setenv("NODEFLOW_SINK", "http")
	config, err := Load()
	if err != nil {
	...
	if config.TopologyEngine != "typescript" || config.Sink != "http" {
		t.Fatalf("unexpected rollback configuration: %#v", config)
	}
}
